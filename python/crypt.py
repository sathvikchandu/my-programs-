def xor(a, b):                              #xor gunction
    ...
    for i in range(1, len(b)):              #takes 4 bits as input and performs xor operation
        if a[i] == b[i]:                    #on last 3 bits as 1st bit is ignored
            result.append('0')
        else:
            result.append('1')
   
    return ''.join(result)                  #returns the resultant xor bit
...
def crc(binary,divisor):
    ...
    binary+="000"
    o= xor(binary[i:b+1],divisor)               #taking 4 characters of string and passing them to the function
    b+=1
    divisor1="0000"
...
if finalres=="000":                      #checking if the final remainder is 000
    ...
    print("NO, your message is corrupted and the position of corruption is "+)