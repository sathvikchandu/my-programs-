...
def encryption(plain_text, key):
    ...
    n = 8      #because the 2**8 makes it 256 which we gets until 255 which is range of the pixel color values 
...
def image_encoder(rgb_as_bin,width,height,mode):
    ...
    if mode == 1:#encryption mode
        cons_img.save('enc_img.bmp')
    else:
        cons_img.save('Data_image.bmp')
...
def main():
    ...
    if choice == 1:
        ...
        print("Encrypted image saved as enc_img.bmp")
    elif choice == 2:
        img_og = Image.open(file_path)
        width, height = img_og.size
        rgb_as_bin = image_decoder(img_og)
        ...
        print("Encrypted image saved as Data_image.bmp")
main()